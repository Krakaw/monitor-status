...
const INDIVIDUAL_EVENT_COLUMN_COUNT =
  parseInt(process.env.INDIVIDUAL_EVENT_COLUMN_COUNT) || 3;
...
function generateColor(milliSecondsUntilEvent) {
  ...
  if (secondsUntilEvent >= HOUR) {
    ...
    const red = Math.min(
      200,
      Math.floor(maxRed * (1 - Math.max(0, remainingInterval))) + minRed
    );
...
async function checkCalendarEvents() {
  ...
  if (PICO_DEV) {
    ...
    if (picoCache !== updatePicoCmd) {
      ...
      const data = JSON.stringify({
        timeSync: getTimeString(),
        dates: calEventsArray,
      });
  ...
  setLed(
    getLed(INDIVIDUAL_EVENT_COLUMN_COUNT),
    ...getPendingColor(pendingCounter)
  );
}
...
async function checkServer({
  ...
  try {
    ...
    if (typeof url === "string") {
      ...
      const body =
        checkParams.type === "json" ? await result.json() : await result.text();
      success =
        checkParams.type === "json"
          ? body[checkParams.key] === checkParams.value
          : body.indexOf(checkParams.value) > -1;
...
function main() {
  ...
  setInterval(() => {
    checkCalendarEvents()
      ...
      .finally(() => {
        const errorMessage = `${statusRgb[0] > 0 && "Server Status Error"} ${
          statusRgb[2] > 0 && "Calendar Check Error"
        }`;
